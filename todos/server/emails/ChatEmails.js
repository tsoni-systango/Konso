var dialogsWithNewMessages = {};

var observerOfMessages = Messages.find({}).observe({
	added: function(doc){
		if(!observerOfMessages) return;
		dialogsWithNewMessages[doc.dialogId] = 1;
	}
});

findAndSendUnreadMessagesToEmails = function () {
	while (Object.keys(dialogsWithNewMessages).length){
		var dialogId = Object.keys(dialogsWithNewMessages)[0];
		delete dialogsWithNewMessages[dialogId];
